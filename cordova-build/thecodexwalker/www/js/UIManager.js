class UIManager {
    ...
    showLoadingError(message) {
        // 更新加载界面的文本显示错误信息
        if (this.loadingText) {
            this.loadingText.textContent = '数据加载失败';
        ...
        // 显示重试提示
        if (this.enterPrompt && this.enterPromptText) {
            this.enterPromptText.textContent = '点击重新加载';
            ...
            // 移除之前的事件监听器（如果有）
            this.screens.initialLoading.removeEventListener('click', retryHandler);
            // 添加新的事件监听器
            this.screens.initialLoading.addEventListener('click', retryHandler);
        }
        
        console.error('显示加载错误:', message);
    }
}
